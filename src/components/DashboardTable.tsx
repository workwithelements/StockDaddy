    ...
    <>
      ...
      >
        ...
        <td className="px-4 py-3 text-right tabular-nums bg-amber-50/30 text-amber-700 font-medium">
          ...
          {(() => {
            const undated = group.variants.reduce(
              (s, v) => s + (v.undatedOnOrder ?? 0),
              0
            );
            return undated > 0 ? (
              <div
                className="text-xs text-red-600 font-normal"
                title={`${undated} units on order have no ETA — not counted in Days Left. Click the product to set arrival dates.`}
              >
                ⚠ {undated} undated
              </div>
            ) : null;
        ...
        group.variants.map((variant, idx) => (
          ...
          >
            ...
            <td className="px-4 py-2 text-right tabular-nums bg-amber-50/20 text-amber-700">
              ...
              {variant.undatedOnOrder > 0 && (
                <div
                  className="text-xs text-red-600"
                  title="Some on-order qty has no ETA and is excluded from Days Left."
                >
                  ⚠ {variant.undatedOnOrder} undated
                </div>
              )}
            </td>
            <td className="px-4 py-2 text-right tabular-nums text-gray-600">
